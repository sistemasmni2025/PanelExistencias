...
const PromoBadge = ({ pdfUrl, className, showIndicator = true }) => {
  if (!pdfUrl) return null;
  // Always ensure absolute and z-index are present
  const badgeClass = className.includes('absolute') ? className : `absolute ${className}`;
  ...
  return (
    <a 
      ...
      className={`${badgeClass} group/badge`}
      title="Ver PDF de la Promoción"
    >
      <svg viewBox="0 0 100 100" className="w-full h-full animate-[spin_12s_linear_infinite] drop-shadow-md">
         <polygon fill="#ffce00" points="50,2 62,22 83,12 80,35 100,45 83,62 90,85 68,80 50,98 32,80 10,85 17,62 0,45 20,35 17,12 38,22" />
         <polygon fill="#d80000" points="50,8 59,25 77,17 74,36 91,45 77,59 82,78 64,74 50,89 36,74 18,78 23,59 9,45 26,36 23,17 41,25" />
      </svg>
      <div className="absolute inset-0 overflow-hidden rounded-full mix-blend-overlay">
        <div className="w-[150%] h-[30%] bg-white/40 -rotate-45 translate-y-[300%] group-hover/badge:-translate-y-[100%] transition-transform duration-700 ease-in-out absolute"></div>
      </div>
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-20 pointer-events-none">
         <span className="text-white font-black text-[13px] leading-none drop-shadow-lg transform -rotate-12 tracking-wide block scale-110 animate-pulse">¡PROMO!</span>
      </div>

      {/* Subtle indicator hand */}
      {showIndicator && (
        <div className="absolute -left-4 top-1/2 -translate-y-1/2 group-hover/badge:opacity-100 opacity-0 transition-opacity animate-bounce-horizontal pointer-events-none z-30">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="drop-shadow-sm">
            <path d="M12 19L19 12M19 12L12 5M19 12H5" stroke="#22c55e" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
      )}
    </a>
        ...
        <div className="p-4 md:p-5 overflow-y-auto custom-scrollbar bg-slate-50 flex-1">
          <div className="flex flex-col lg:flex-row gap-5 h-full">
            ...
            <div className="w-full lg:w-[280px] shrink-0 flex flex-col gap-3">
              <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex items-center justify-center relative flex-1 min-h-[220px] overflow-hidden group/modalimg">
                {product.campaignPdf && (
                  <PromoBadge 
                    pdfUrl={product.campaignPdf} 
                    className="absolute -top-3 -right-3 z-50 w-20 h-20 hover:scale-110 transition-transform cursor-pointer drop-shadow-2xl" 
                  />
                )}
...
const ProductCard = ({ product, onAddToCart, onClick }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col relative group mt-2 mr-3">
      {/* Image Area - Clickable */}
      <div
        className="relative aspect-[4/3] bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-6 border-b border-slate-100 overflow-hidden cursor-pointer rounded-t-2xl"
      ...
      <div className="p-5 flex flex-col flex-1 relative">
        {product.campaignPdf && (
          <PromoBadge 
            pdfUrl={product.campaignPdf} 
            className="absolute top-4 right-3 z-50 w-16 h-16 group/badge hover:scale-110 transition-transform cursor-pointer drop-shadow-xl" 
          />
        )}
        {/* Title - Clickable */}
        <h3
          className={`text-[14px] lg:text-[15px] font-black text-slate-800 leading-snug mb-3 line-clamp-2 min-h-[46px] group-hover:text-[#003d7a] transition-colors relative z-10 cursor-pointer ${product.campaignPdf ? 'pr-14' : 'pr-1'}`}
